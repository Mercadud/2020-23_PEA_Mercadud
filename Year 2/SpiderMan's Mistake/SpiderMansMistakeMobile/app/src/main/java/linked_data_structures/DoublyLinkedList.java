...
public class DoublyLinkedList<E> implements Serializable {
	 ...
	 * @param b
	 */
	public void swapNodesBesideEachOther(int a, int b) throws Exception {
		if (a != b - 1) {
			throw new Exception("They are not beside each other");
		}
		DLNode<E> el1 = find(a);
		DLNode<E> el2 = find(b);

		DLNode<E> firstPred = el1.getPredecessor();
		DLNode<E> secSuc = (DLNode<E>) el2.getSuccessor();

		el1.setPredecessor(el2);
		el1.setSuccessor(secSuc);
		secSuc.setPredecessor(el1);
		el2.setPredecessor(firstPred);
		el2.setSuccessor(el1);
		firstPred.setSuccessor(el2);
	}

	public void replaceElementAt(int p, E el) {
		find(p).setElement(el);
	}
